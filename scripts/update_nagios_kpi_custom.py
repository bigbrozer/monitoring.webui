...
try:
	...
	start_timestamp = int(time.mktime(last_kpi.date.timetuple()))

try:
	end_timestamp = sys.argv[2]
except IndexError:
	last_kpi = NagiosKPI.objects.order_by('-date')[0]
	end_timestamp = int(time.mktime(last_kpi.date.timetuple())) + 604800
...
kpi = NagiosKPI()
kpi.date = datetime.fromtimestamp(float(end_timestamp))
